...
class _SessionPageState extends State<SessionPage> {
  ...
  Future<void> _deleteTurn(TurnModel turn) async {
    ...
    try {
      final res  = await TurnRequests().delete(
          widget.token.token, widget.club.id, turn.id);
      final body = jsonDecode(res.body);
      if (body['success'] == true) {
        await LocalDatabase().deleteTurn(turn.id);
        _loadTurns();
      } else {
        _showError(body['message'] ?? 'Could not delete turn');
      }
    } catch (e) {
  ...
  Future<void> _completeTurn(TurnModel turn) async {
    try {
      final res  = await TurnRequests().complete(
          widget.token.token, widget.club.id, turn.id, 1);
      final body = jsonDecode(res.body);
      if (body['success'] == true) {
        final updated = turn.copyWith(completed: true);
        await LocalDatabase().upsertTurn(updated);
        _loadTurns();
      } else {
        _showError(body['message'] ?? 'Could not mark turn as done');
      }
    } catch (e) {
      _showError(e.toString());
    }
  ...
  Future<void> _toggleSessionComplete() async {
    ...
    try {
      ...
      final body = jsonDecode(res.body);
      if (body['success'] == true) {
        final updated = _session.copyWith(completed: newValue == 1);
        await LocalDatabase().upsertSession(updated);
        setState(() => _session = updated);
      } else {
        _showError(body['message'] ?? 'Could not update session');
      }
    } catch (e) {
      _showError(e.toString());
    }
            ...
            child: Column(
              ...
              children: [
                ...
                TextFormField(
                  ...
                  validator: (v) =>
                      (v == null || v.isEmpty) ? 'Pick a start date' : null,
                ),
                ...
                TextFormField(
                  ...
                  validator: (v) =>
                      (v == null || v.isEmpty) ? 'Pick an end date' : null,
                ),
                        ...
                        onPressed: () async {
                          ...
                          try {
                            ...
                            if (body['success'] == true) {
                              ...
                              _showError(
                                  body['message'] ?? 'Could not create turn');
                            }
                          } catch (e) {
                            _showError(e.toString());
                          } finally {
